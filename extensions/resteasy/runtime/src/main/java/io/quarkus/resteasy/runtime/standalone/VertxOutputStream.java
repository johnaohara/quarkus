...
public class VertxOutputStream extends AsyncOutputStream {
    ...
    public CompletionStage<Void> rxFlush() {
        return rxFlush(false);
    }

    private CompletionStage<Void> rxFlush(boolean isLast) {
        if (closed) {
    ...
    public CompletionStage<Void> rxWrite(final byte[] b, final int off, final int len) {
        ...
        return ret.thenCompose(v -> rxUpdateWritten(len));
    }

    CompletionStage<Void> rxUpdateWritten(final long len) {
        this.written += len;
        if (contentLength != -1 && this.written >= contentLength) {
            return rxFlush(true).thenAccept(v -> {
                closed = true;
            });
        }
        return CompletableFuture.completedFuture(null);
